...
data = data.reshape([len(data), 1024, -1])
model = tf.keras.models.Sequential()
model.add(tf.keras.layers.BatchNormalization(input_shape=data.shape[1:]))
model.add(tf.keras.layers.Conv1D(1024, 32, padding='same', activation='elu'))
